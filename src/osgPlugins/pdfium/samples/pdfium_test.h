// Copyright (C) 2021 Christian Frisson
// Adapted from samples/pdfium_test.cc from PDFium
//
...
struct Options {
  ...
  bool use_load_mem_document = true; // for ReaderWriterPDFium
  bool render_oneshot = true; // for ReaderWriterPDFium
  bool lcd_text = false;
...
int ExampleDocGetFilePath(IPDF_JSPLATFORM*, void* file_path, int length) {
  ...
  // constexpr 
  int kRequired = static_cast<int>(sizeof(kPath));
...
// Convenience pdfium_reader class for ReaderWriterPDFium
class pdfium_reader {
  public:
    pdfium_reader()
    : width(0),
  ...
  public:
    ...
    FPDF_BITMAP getBitmap(){return bitmap.get();}
...
bool ProcessPage(const std::string& name,
  ...
  // auto 
  width = static_cast<int>(FPDF_GetPageWidthF(page) * scale);
  // auto 
  height = static_cast<int>(FPDF_GetPageHeightF(page) * scale);
  int alpha = FPDFPage_HasTransparency(page) ? 1 : 0;
  bitmap = ScopedFPDFBitmap (FPDFBitmap_Create(width, height, alpha));
  ...
  if (bitmap) {
    ...
    // int 
    stride = FPDFBitmap_GetStride(bitmap.get());
    void* 
    buffer = FPDFBitmap_GetBuffer(bitmap.get());
...
void ProcessPdf(const std::string& name,
  ...
  // int 
  page_count = FPDF_GetPageCount(doc.get());
